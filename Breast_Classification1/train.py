...
def test(testloader, model, criterion, epoch, use_cuda):
    ...
    #         acc, prec, rec, f1, auc = classification_report(outputs.data,targets.data)
    #         s = inputs.size(0)
    #         precision.update(prec.item(), s)
    #         recall.update(rec.item(), s)
    #         auc_roc.update(auc.item(), s)
    #         f1_score.update(f1.item(),s)
...
def main():
    ...
    transform_medical = transforms.Compose([
        ...
        transforms.Grayscale(num_output_channels=3),
        # transforms.Resize(32),  # reize 256x256 
